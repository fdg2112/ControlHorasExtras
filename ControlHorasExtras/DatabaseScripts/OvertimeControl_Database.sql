...
('intendente', '123', 'Hernan', 'Besel', 4, NULL, NULL);
...
BEGIN
    IF EXISTS (
        ...
        FROM inserted i
        JOIN Areas a ON i.AreaID = a.AreaID
        WHERE i.SecretariaID <> a.SecretariaID AND i.AreaID IS NOT NULL
    )
    BEGIN
        ROLLBACK TRANSACTION;
        THROW 50002, 'Error: El AreaID especificado no pertenece a la SecretariaID asignada.', 1;
    END
END;
GO
...
DECLARE @MaxHours50 INT = 10;
DECLARE @MaxHours100 INT = 5;
...
BEGIN
    ...
    BEGIN
        ...
        SET @RandomHours = FLOOR(RAND() * (@MaxHours50 + 1));
        IF @RandomHours > @MaxHours100 
        BEGIN
            SET @RandomHours = @MaxHours100;
        END

        SET @StartTime = DATEADD(HOUR, FLOOR(RAND() * 12), @CurrentDate);
        SET @EndTime = DATEADD(HOUR, @RandomHours, @StartTime);
		SET @ActividadID = FLOOR(RAND() * 4) + 1;

        -- Determino el tipo de hora
